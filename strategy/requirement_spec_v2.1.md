...
| 규칙 ID | 요구사항 | 검증 방법 |
|:---:|:---|:---|
| REQ-3.3.1 | 테이블 헤더에 그라데이션 또는 배경색 적용 | `thead th` 또는 `th`에 `background` 스타일 존재 |
| REQ-3.3.2 | Zebra Striping (홀짝 행 구분) | `tr:nth-child` CSS 규칙 또는 교대 배경색 확인 |
| REQ-3.3.3 | `border-radius` 적용 (Elevated 효과) | `table`에 `border-radius` 스타일 존재 |
| REQ-3.3.4 | `box-shadow` 적용 | `table`에 `box-shadow` 스타일 존재 |
| REQ-3.3.5 | 타이포그래피 Montserrat 폰트 적용 | CSS 내 `Montserrat` 폰트 참조 확인 |
...
|:---:|:---|:---|
| REQ-4.1.1 | 모든 이미지 경로는 **상대 경로** (`../images/`) | `img[src^='../images/']` |
| REQ-4.1.2 | 절대 경로(`/Users/`, `C:\`, `http`) 사용 금지 | `img[src]` 값에 절대 경로 패턴 없음 |
| REQ-4.1.3 | 이미지 내 **일본어/한자** 등 불필요한 언어 없음 | 수동 검증 (자동화 불가) |
| REQ-4.1.4 | 이미지 파일이 `images/` 폴더에 실제 존재 | 파일 시스템 존재 여부 확인 |

**테스트 케이스**:

- **TC-4.1.1**: 모든 `<img>` 요소의 `src` 속성값이 `../images/`로 시작.
- **TC-4.1.2**: 어떤 `<img>` 요소도 `src` 값에 `/Users/`, `http://`, `https://`, `C:\` 포함하지 않음.
- **TC-4.1.3**: 각 `img[src]`의 실제 파일이 `images/` 폴더에 존재하는지 파일 시스템 확인.

### REQ-4.2: 섹션별 이미지 필수 규칙

| 규칙 ID | 대상 섹션 | 요구사항 |
|:---:|:---|:---|
| REQ-4.2.1 | **2. Vocabulario Esencial** | 이미지 1개 이상 필수. 어휘 내용 100% 반영, 어휘 넘버가 삽화에 빠짐없이 표시 |
| REQ-4.2.2 | **5. Cultura Viva** | 이미지 1개 이상 필수. 섹션 내용을 최대한 반영 |
